...
from build_assignment import build_assignment
...
def compute_numeric_jacobian(x, x_shape, x_data, y, y_shape, image_label, image_label_value, delta=0.001,
  ...
  for row in range(x_size):
    ...
    new_y = build_assignment(x, image_label)
...
class BuildAssignmentTest(tf.test.TestCase):
  def test(self):
    with tf.Session() as sess:
      x_array = np.zeros((16,2,2,3))
      for i in range(2):
        for j in range(2):
          for k in range(3):
            x_array[0,i,j,k] = (i+1)*(j+1)*(k+1);
      x = tf.constant(x_array, tf.float32) 

      y_array = np.zeros((16,2,2))
      y_array[0,...] = np.array([[0,1],[2,0]])
      ...
      self.assertLess(np.sum(np.abs(cost_matrix_val[0,:,:] - [[5,2,2],[10,4,4],[15,6,6]])), 1e-4)

  def test_grad(self):
    with self.test_session():
      x = tf.constant(np.random.rand(16,2,2,3), dtype=tf.float32)
      y = tf.constant(np.random.randint(low=0, high=3, size=(16,2,2)), dtype=tf.int32)

      cost_matrix = build_assignment(x, y)
      err = tf.test.compute_gradient_error(x, (16,2,2,3), cost_matrix, (16,3,3))
      print 'error:', err
